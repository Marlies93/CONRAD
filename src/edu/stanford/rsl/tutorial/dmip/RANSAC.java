...
public class RANSAC {
	...
	public SimpleVector commonRansac(SimpleMatrix points, int mn, double p_opt, double p_out){
		...
		for(int i = 0; i < it; i++)
		{
			...
			for(int n = 1; n < mn; n++){
				...
				// check if the point was already picked
		...
		// => RANSAC ignores the outliers to form the regression line. 
	}
	...
	public double lineError(SimpleVector line_params, SimpleMatrix points){
		...
		// Blackboard: We use the Hessian Normal Form to calculate the error: x * n = d (distance)
		// with n = (-m 1)' and d = p * n instead of y = m*x + t. => distance point
		// to line: x_p * n - d
		
		// Threshold defining the allowed distance of a point to the line
		// All points inside are ok, all points outside are counted as an error
		double thresh = 0.2;
		...
		// TODO: get some point on the line (here x = 1)
